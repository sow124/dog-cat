...
function Cat(){
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 이전에 받아온 데이터 중 가장 최신의 날짜를 구합니다.
        const latestDate = animals.length > 0 ? animals[0].formattedhappenDt : null;
        ...
        const response = await fetch(`http://apis.data.go.kr/1543061/abandonmentPublicSrvc/abandonmentPublic?bgnde=20220508&endde=${today}&upkind=422400&pageNo=${page}&numOfRows=10&ServiceKey=${API_KEY}`);
  ...
  return(
    <div className="container">
      <h2 className="dogTitle">유기묘</h2>
      <div className="aniamlContainer">
      {animals.map((animal,index) =>(
          <div className="aniamlDetailCon" key={index}>
            <div className="animalCard">
            <Link to={{pathname: `/${animal.desertionNo}`,search: `?animals=${encodeURIComponent(JSON.stringify(animal))}`,}}
